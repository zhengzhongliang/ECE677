...
def MLP(x):
  ...
  with tf.variable_scope('layer1'), tf.device('/cpu:0'):
    ...
    W1 = tf.get_variable(name='weight1', shape= [784, 50], initializer=tf.truncated_normal_initializer(seed=SEED))
    b1= tf.get_variable(name='bias1',shape=[50], initializer=tf.truncated_normal_initializer(seed=SEED))
    H=tf.add(tf.matmul(x, W1), b1)
    A = tf.nn.relu(H)
    W2 = tf.get_variable(name='weight2', shape=[50, 10], initializer=tf.truncated_normal_initializer(seed=SEED))
    b2 = tf.get_variable(name='bias2', shape=[10], initializer=tf.truncated_normal_initializer(seed=SEED))
    y = tf.matmul(A, W2) + b2
    print('shape:',y.shape)
